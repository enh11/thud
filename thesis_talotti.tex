...
Now we have to decide which Weierstrass equations define isomorphic elliptic curves. We can restrict ourselves to isomorphisms that fix the point at infinity. We shall continue to assume that the characteristic of $\K$ is prime to 6, so we can deal with short Weierstrass form. However completely analogous discussions can be done for characteristics 2 and 3 and can be found in \cite{Sil2}. We look for invertible transformation of the affine coordinates for
...
Now we want to translate the results of proposition \ref{classificazione1}  into "invariants" of $E$ that	can be read off from any Weierstrass equation. As stated in the definition of elliptic curves, it has to be smooth, i.e., the roots of the cubic in the right-side of the equation \ref{ShortWeierstrass} must be simple. This append if and only if the discriminant of the cubic is not equal to zero. 
\begin{definizione}
...
\end{definizione}
\begin{teorema}
	Let $\K$ be a field of characteristic prime to 6 and let $E:y^2=x^3+a_4x+a_6$, $E':y'^2=x'^3+a_4'x'+a_6'$ be two elliptic curves over $\K$ with $j$-invariants $j,\ j'$ respectively. If $j=j'$, then there exists $u\in\clK^*$ such that $$a_4'=u^4a_4,\qquad a_6'=u^6a_6.$$  The transformation $$x\mapsto x'=u^2x,\quad y\mapsto y'=u^3y$$ takes one equation to the other.
\end{teorema}
\begin{proof}
Assume $a_4\neq0$. Then we have $j'=j\neq 0$, thus $a_4'\neq 0$. Choose $u\in\clK$ such that $a_4'=u^4a_4$, then we have 
$$\dfrac{4a_4^3}{4a_4^3 +27a_6^2}=\dfrac{4a_4'^3}{4a_4'^3 +27a_6'^2}=\dfrac{4u^{12}a_4^3}{4u^{12}a_4^3 +27a_6'^2}=\dfrac{4a_4^3}{4a_4^3 +27u^{-12}a_6'^2},$$ which implies that $a_6'^2=\pm u^6a_6$. If $a_6'^2=u^6a_6$, we're done. Otherwise we can change $u$ to $iu$, where $i^2=-1$. This preserves the relation $a_4'=u^4a_4$ and
also yields $a_6'^2=u^6a_6$. 

If $a_4=0$, then also $a_4'=0$ and since $\Delta\neq 0\neq \Delta'$ we have $a_6\neq 0\neq a_6'$. Thus we can choose $u\in\clK$ such that $a_6'=u^6a_6$.
\end{proof}
Note that the $j$-invariant tells us when two curves are isomorphic over an algebraically closed field. However, if we are working with a non-algebraically
closed field K, then it is possible to have two curves with the same $j$-invariant that cannot be transformed into each other by any transformation with coefficients in $\K$, but we can use an extension field of $\K$. In agreement with \ref{classificazione2}, two curves with the same $j$-invariant are said to be $twist$ of each other.
\begin{corollario}
Assume that the characteristic of $\K$ is prime to 6 and let $E: y^2 = x^3 + a_4x + a_6.$ The absolute invariant $j_E$ depends only on the isomorphism class of $E$.	
\begin{itemize}
...
\end{proposizione}
\begin{osservazione}\label{Kpunti}
	 When we consider a point $P$ on $E/\K$, it is implicit that $P$ has its coordinates in $\clK$; we denote this set by $E(\clK)$. The points lying on $E$ with coordinates in the base field $\K$ form the set of $\K-rational$ $points$ $of$ $E$ and we denote it by $E(\K)$. Thus we have $E(\K)=\left\{(x_1,y_1)\in\K^2\ :\ y_1^2+a_1y_1+a_3y_1=x_1^3+a_2x_1^2+a_4x_1+a_6\right\}$. In general, for any field extension $\K\subset\mathbb{L}\subset\clK$ we denote by $E(\mathcal{L})$ the set of $\mathcal{L}-points$ of $E$. As already stated, $E(\clK)$ carries the structure of an abelian group and the sets of $\L$- points, as well as the $K$-rational points, are subgroup of $E(\clK)$.
\end{osservazione}
%\begin{esempio}
...
Weierstrass form; for more details we refer to \cite{Wash},\cite{Coh1},\cite{mong}. Then we introduce some curves we will need for our analysis. They are recommended by NIST \cite{nist} and are of common use in cryptography. In next chapters we will study some weakness of these curves.
...
\section{Some example of NIST recommended curves}
...
\begin{itemize}
	...
	\item The curve has the Weierstrass form $y^2 = x^3+7,$ where $b\in\Fq$ is generated by a public deterministic algorithm.
...
is significantly smaller than the order n of the base-point $G$ of P-224. As a result, it is essential to verify domain parameter validity
...
In the first chapter we saw that the set of points of an elliptic curve is a finite abelian group. We also described some algorithm scalar multiplication as well as some technique to speed up the computation. In a general setting, however, the inverse operation, called $discrete$ $logarithm$, is not as fast; there is no known algorithm that can compute discrete logarithm in $E(\Fq)$ much faster than $O(\sqrt{p})$  where $p$ is the order of $P$ in $E(\Fq)$. This makes the scalar multiplication a one way function from the group $E(\Fq)$ to itself and we can build the $public$ $key$ $elliptic$ $curve$ $cryptosystem.$ 

In this chapter we present an algorithm to solve the discrete logarithm problem. Such an algorithm can be use to determine whether a public key comes from a weak private key subject to a given computational bound, and if so, recover the private key from the corresponding public key. We follow the work of M. J. Jacobson and P. Kushwaha, \cite{Pra1},\cite{Pra2} whose result are inspired by the work of Maurer and Wolf \cite{Mau}.
\section{Discrete logarithm problem}
Since our main focus are elliptic curves, we write our group $G$ additively. Results applies in more general cases $mutatis$ $mutandis$.
\begin{definizione}
Let $(G,\oplus)$ be an additive cyclic group of prime order $p$ and generator $P$. The map
\begin{align*}
	\varphi:\ &\Z\ \to\ G\\
	&n\ \mapsto\ [n]P=\underbrace{P\oplus P\oplus\dots\oplus P}_{n\ \text{times}}
\end{align*}
has kernel $p\Z$, thus $\varphi$ leads to an isomorphism between $(G,\oplus)$ and $(\Z/p\Z,+):=\Z_p.$ The problem of computing the inverse map is called the $discrete$ $logarithm$ $problem$ ($DLP$) to the base of $P$. It is the problem, given $P$ and $Q$, to determine $k\in\Z$ such that $Q = [k]P$. Note, that it $k$ is
unique only modulo $p$ where $p$ is the group order.
\end{definizione}
The complexity of this problem depends on the choice of $G$ its operation. If $G=\Z_p$ with generator $1$, the discrete logarithm of $n\in\Z_p$
is $n$ it self. Also, if the generator is chosen to be $a\in\Z_p$, this problem is easy to solve as it is nothing but computing the inverse modulo $p$.
\section{Baby step giant step algorithm}
The baby-step giant-step algorithm was first published by Shanks,the first application of this method was to compute ideal class numbers of quadratic number fields \cite{Sha},\cite{Coh2}. It can be used for discrete logarithm computation and we shall present it in this form, which is more general. Order computation is then solving $[n]P=\infp$ with $n\neq 0$ (and of course with unknown order). The baby-step giant-step method is based on the following observation: 
\begin{lemma}
...
\end{lemma}
\begin{proof}
	If we divide $\alpha$ by $m$ we have $\alpha=km+i$ with $0\leq i\leq m-1$. We note that $\alpha\leq n-1 = m^2-1=m(m-1)+(m-1)$ and we know that $i\leq m-1$ so it follow that $0\leq j\leq m-1$
\end{proof}
Assume now that the order of $P\in G$ is $n$ and $k$ an integer modulus $n$. Let $Q=[k]P$, then we have $$Q\oplus[-jm]P=[i]P$$ for some $i,j$ as in the lemma. We have the following.
\begin{proposizione}
	Let $(G,\oplus)$ be a finite, additive, cyclic group of order $n$ and let $P$ be a
	generator of $G$. The following algorithm solves the discrete logarithm problem $[\alpha]P = Q$ in $O( n \log n)$ steps.
\begin{enumerate}
	\item Let $m=\lfloor n\rfloor +1$.
...
\end{proposizione}
\begin{proof}
	Calculating the two lists takes at most $2m$ group operations. By the lemma above there exists a match between the two lists, whose can be found in $\log m$ steps by using standard searching algorithms or hash tables. Hence the total running	time for the algorithm is $O( n \log n)$ steps.
\end{proof}
