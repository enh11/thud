...
We used the computer algebra system pari-gp \cite{pari} for computations. In Listing \ref{gpcode} you can find scripts for computing $n_B$, $c_B$ for the curve secp256k1 within a bound $B=2^{32}$. We can deduce the weak keys analysis for other NIST's curves using the value of $p$ presented in the section \ref{NISTcurves}. As for other curves, one can find parameters in \cite{Crocs}.
...
Here we give an assessment of weak keys in recommentded and standard elliptic curves together with the worst-case number of scalar multiplications required to detect them. Table \ref{weakprime} summarize the weak key analysis for curves presented in section \ref{NISTcurves}. Curves in table \ref{weakbin} are all defined over a binary field while curves in table \ref{weakwtls} are Wireless Transport Layer Security curves.  Note that the time required to count weak keys for a particular curve is dominated by the computation of the divisors of $p-1$, and thus by factoring. Therefore, a complete analysis for all standardized curves used by the community can be performed in several days; a complete analysis for other standardized curves is available in \cite{Pra2}.
	\begin{table}
