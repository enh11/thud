...
Table \ref{weakkeysanal} summarize the weak key analysis for curves presented in section \ref{NISTcurves}. Curves in table \ref{weakbinaryanalysis} are all defined over a binary field while curves in table \ref{wtls} are Wireless Transport Layer Security curves.  Note that the time required to count weak keys for a particular curve is dominated by the computation of the divisors of $p-1$, and thus by factoring. Therefore, a complete analysis for all standardized curves used by the community can be performed in several days; a complete analysis for other standardized curves is available in \cite{Pra2}.
